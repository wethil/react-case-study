...
import type { SortColumn, SortState, TableColumn } from "@/types/table.types";
...
const getSortIcon = (column: SortColumn, sort: SortState) => {
    ...
    <tr>
      {columns.map((col) => (
        <th
          ...
          onClick={
            col.sortable ? () => onSort(col.key as SortColumn) : undefined
          }
          aria-sort={
          ...
          onKeyDown={
            col.sortable
              ? (e) => e.key === "Enter" && onSort(col.key as SortColumn)
              : undefined
          }
        >
          <span className="flex items-center">
            {col.label}
            {col.sortable && getSortIcon(col.key as SortColumn, sort)}
          </span>
        </th>
      ))}
    </tr>
